...
void RobotController::update_plan(RobotStates &state, double dt) {
    ...
    if (!state.movement_mode) {
        ...
        for (bool &plan_contact: state.plan_contacts) plan_contact = true; //plan_contacts set true --> all feet in contact with ground
        state.gait_counter_reset(); //counter reset
    } else {
        // movement_mode == 1, walk
        for (int i = 0; i < NUM_LEG; ++i) //each leg   --> 땅에 닿아있는 다리와 그렇지 않은 다리 구별 및 gait cycle 확인
        {
            state.gait_counter(i) = state.gait_counter(i) + state.gait_counter_speed(i); // gait counter cycle update && gait_counter로 다리 제어 (swing <-> stance) --> 초기 gait_counter가 다리별로 다르더라
            state.gait_counter(i) = std::fmod(state.gait_counter(i), state.counter_per_gait); // counter_per_gait --> whole gait cycle --> counter_per_gait를 넘지 않도록 조정
            if (state.gait_counter(i) <= state.counter_per_swing)  // gait_counter <=  counter_per_swing --> ground contact
            {
...
void RobotController::generate_swing_legs_ctrl(RobotStates &state, double dt) // swing force calculate
{
    ...
    for (int i = 0; i < NUM_LEG; ++i) {
        ...
        // from foot_pos_cur to foot_pos_final computes an intermediate point using BezierUtils // 베지어 곡선으로 다리 궤적 부드럽게 계산
...
void RobotController::compute_joint_torques(RobotStates &state) {
    ...
    if (mpc_init_counter < 100) {
        ...
        //gait counter에 의해 leg trajectory가 그려지고, 그 trajectory에 따라 이동, 공중에 뜬 trajectory일 때는 swing leg force 제어, 땅에 닿아있을 때는 mpc 제어
        //gait에 의해 공중에 뜨는 다리와 그렇지 않은 다리 판별함
        for (int i = 0; i < NUM_LEG; ++i) {
...
Eigen::Matrix<double, 3, NUM_LEG> RobotController::compute_grf(RobotStates &state, double dt) {
    ...
    if (state.root_pos[2] > 0.1) {
        terrain_angle = terrain_angle_filter.CalculateAverage(Utils::cal_dihedral_angle(flat_ground_coef, surf_coef)); // coef 사이 각도 계산
    } else {
        terrain_angle = 0;
    }
    ...
    for (int i = 0; i < PLAN_HORIZON; i++) {
        ...
        mpc_solver.calculate_B_c(state.robot_mass,
    ...
    Eigen::Map<Eigen::Matrix<qpOASES::real_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(H, nV, nV) = mpc_solver.hessian;
    Eigen::Map<Eigen::Matrix<qpOASES::real_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(A, nC, nV) = mpc_solver.linear_constraints;
    Eigen::Map<Eigen::Matrix<qpOASES::real_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(g, nV, 1) = mpc_solver.gradient;
    Eigen::Map<Eigen::Matrix<qpOASES::real_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(lbA, nC, 1) = mpc_solver.lb;
    Eigen::Map<Eigen::Matrix<qpOASES::real_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(ubA, nC, 1) = mpc_solver.ub;
