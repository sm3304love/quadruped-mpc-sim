...
void ConvexMpc::calculate_A_c(Eigen::Vector3d root_euler)
{
    ...
    ang_vel_to_rpy_rate << cos_yaw, sin_yaw, 0, -sin_yaw, cos_yaw, 0, 0, 0, 1;
                              ...
                              Eigen::Matrix<double, 3, NUM_LEG> foot_pos)
{
    ...
    {
        B_mat_c.block<3, 3>(6, 3 * i) = trunk_inertia_world.inverse() * Utils::skew(foot_pos.block<3, 1>(0, i));
...
void ConvexMpc::calculate_mpc_matrix(RobotStates &state) // qp로 풀기
{
    ...
    for (int i = 0; i < NUM_LEG; ++i)
    {
        lb_one_horizon.segment<5>(i * 5) << 0, -qpOASES::INFTY, 0, -qpOASES::INFTY, f_min * state.contacts[i];
        ub_one_horizon.segment<5>(i * 5) << qpOASES::INFTY, 0, qpOASES::INFTY, 0, f_max * state.contacts[i];
    }
    for (int i = 0; i < PLAN_HORIZON; ++i)
    {
