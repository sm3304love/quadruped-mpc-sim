...
EKFFilter::EKFFilter(bool assume_flat_ground_) : EKFFilter()
{
    // constructor
...
void EKFFilter::init_state(RobotStates &state)
{
...
void EKFFilter::update_estimation(RobotStates &state, double dt)
{
    ...
    for (int i = 0; i < NUM_LEG; ++i)
    {
        ...
        y.block<3, 1>(NUM_LEG * 3 + i * 3, 0) = (1.0 - estimated_contacts[i]) * x.segment<3>(3) +
                                                estimated_contacts[i] * state.root_rot_mat * leg_v; // vel estimation
    ...
    // final step
    // put estimated values back to RobotStates& state
    for (int i = 0; i < NUM_LEG; ++i)
    {
